...
class WorkflowStartRequest(BaseModel):
    ...
    raw_text: str = Field(..., json_schema_extra={
                          "example": "Patient John Doe has back pain. Insurance: BCBS."})
...
async def run_workflow_pipeline(workflow_id: str, patient_id: str, raw_text: str, created_by: str):
    """Triggers the remote AI agent to process the prior authorization workflow."""
    ...
    try:
        ...
        # Call remote AI API to process the workflow
        result = await ai_client.trigger_workflow(workflow_id=workflow_id)

        if result.get("status") == "failed":
            raise Exception(result.get("error", "Unknown error from AI client"))

        # Simulate pipeline result parsing or handle response
        # In a real setup, AI agent updates the DB directly or returns details:
        # e.g., result_data can contain { "payer_type": "...", "quality_score": 85.0 }
        prior_auth_form = result.get("prior_auth_form", {
            "fields": {
                "diagnosis_code": "M54.5",
                "procedure_code": "97110",
                "prior_treatments": "NSAIDs",
        ...
        updates = {
            "payer_type": result.get("payer_type", "Aetna"),
            "quality_score": result.get("quality_score", 90.0),
            "prior_auth_form": prior_auth_form,
            "icd10_codes": result.get("icd10_codes", ["M54.5", "M51.36"]),
            "summary": result.get("summary", "Herniated lumbar disc at L4-L5 with radiculopathy."),
            "confidence_score": result.get("confidence_score", 0.95),
        }

        await update_state("awaiting_approval", updates)

    except Exception as e:
        logger.error(f"Error in workflow remote run: {e}", exc_info=True)
...
async def stream_workflow_progress(
    ...
    async def event_generator():
        try:
            ...
            active_streams.pop(wf_id_str, None)
            logger.info(
                f"SSE stream client disconnected for workflow {wf_id_str}")
...
async def approve_workflow(
    ...
    if wf.status != WorkflowStatus.AWAITING_APPROVAL:
        raise HTTPException(
            status_code=400, detail="Workflow is not in awaiting_approval state")
...
async def update_workflow_fields(
    ...
    if not wf.result_data:
        raise HTTPException(
            status_code=400, detail="Workflow does not have result data")
    ...
    confidence = clinical_record.confidence_score if (
        clinical_record and clinical_record.confidence_score is not None) else 1.0
...
async def export_workflow_pdf(
    ...
    if payer_type:
        ...
        if not template:
            result = await db.execute(
                select(PATemplate).where(PATemplate.is_active)
            )
    ...
    if template:
        try:
            ...
            if isinstance(template.fields, dict):
                for field_key, field_val in template.fields.items():
                    if isinstance(field_val, dict) and "x" in field_val and "y" in field_val:
                        field_coords[field_key] = (
                            float(field_val["x"]), float(field_val["y"]))
                ...
                for item in template.fields:
                    if isinstance(item, dict) and "name" in item and "x" in item and "y" in item:
                        field_coords[item["name"]] = (
                            float(item["x"]), float(item["y"]))

            wf_metadata = wf.result_data.get(
                "metadata", {}) if wf.result_data else {}
            wf_coords = wf_metadata.get("coordinates", {}) if isinstance(
                wf_metadata, dict) else {}
            if isinstance(wf_coords, dict):
                for field_key, field_val in wf_coords.items():
                    if isinstance(field_val, (list, tuple)) and len(field_val) == 2:
                        field_coords[field_key] = (
                            float(field_val[0]), float(field_val[1]))
            ...
            return StreamingResponse(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=prior_auth_{id}.pdf"}
            )
        except Exception as e:
            logger.error(
                f"Failed to generate template-based PDF: {e}", exc_info=True)
